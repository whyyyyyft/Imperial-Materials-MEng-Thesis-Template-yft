...
The core of the problem lies with the sheer size of the ground-state wavefunction, which scales exponentially with the number of electrons in a system (to be exact, by a power of 3N). While numerical methods such as quantum Monte Carlo exist, they tend to be exceedingly demanding in terms of computational cost\autocite{foulkesQuantumMonteCarlo2001}. As a result, the majority of electronic structure methods today must simplify the many-electron wavefunction, whether by using an independent-electron approximation (such as the Hartree-Fock method); only considering near(est) neighbours (the myriad tight-binding methods); or by computing the ground-state electronic density rather than the wavefunction.
...
All DFT calculations were performed in OpenMX unless stated otherwise. OpenMX is an open-source DFT software utilizing optimized pseudo-atomic basis functions, and was used to prepare the training data for DeepH-E3. DeepH-E3 is compatible with three DFT softwares, all based on molecular orbitals: OpenMX, . Of the three, OpenMX was chosen as it had already been used in . 
...
Reference bandstructures were also. For the nanotubes, structural relaxations were carried out to within xxxx and relaxed only at the $\Gamma$-point. Nanotubes with radii of <30\,\AA\ (<180 atoms) 
...
For the calculations of dielectric function, absorbance spectra, and joint density of states the \textit{epsilon.x} programme in QuantumESPRESSO was used. The formulae of each are detailed below.
...
All credit goes to Professor Johannes Lischner who derived this in a meeting (thank you very much it was enlightening). 
...
Disregarding the intraband contribution, the equation used for the complex dielectric function was: $$\epsilon_{\alpha, \beta} = \delta_{\alpha\beta} +\frac{8}{N_k\Omega}\sum_{\vec k, m\neq n} \frac{\mathbf{M}_{\alpha, \beta}}{E_{m,k}-E_{n,k}}\frac{f(E_k,n)}{(E_{m,k}-E_{n,k})^2-E^2-i\Gamma E}$$ where $\mathbf{M}_{\alpha, \beta} = \langle$ \red{would like to discuss next meeting! still slightly off but looks promising?}
